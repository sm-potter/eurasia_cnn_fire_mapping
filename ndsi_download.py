...
# max_snow_day.getInfo()


# # In[ ]:


# all_days_vi_image.getInfo()
...
# all_days_done = [int(i) for i in all_days_done
...
# print(max_value.getInfo()['calDoy'])
...
# print(min_value.getInfo()['calDoy'])
...
# data = max_snow_day.reduceRegion(
...
# df = pd.DataFrame(data_array, columns= ['First'])
...
# # Use reduceRegion to get the pixel values within the ROI
# data = max_snow_day_fall.reduceRegion(
#     reducer=ee.Reducer.toList(),
#     geometry=final_buffer2,
#     scale=scale,
#     maxPixels=1e13
...
# # Convert the NumPy array to a Pandas DataFrame column
# df2 = pd.DataFrame(data_array, columns= ['last'])
...
#             while current_value + increment <= max_value:
#                 list1.append(current_value)
#                 list2.append(current_value + increment)
...
#                 post_start_snow = f"{snow_post_year}-{snow_post_month_start}-{snow_post_day_start}"
#                 post_end_snow = f"{snow_post_year}-{snow_post_month_end}-{snow_post_day_end}"
#                 print(pre_start_snow, pre_end_snow, post_start_snow, post_end_snow)
#     #                             print(f"Post start is {post_start_snow}")
#     #                             print(f"Post end is {post_end_snow}")
...
#     #append the old composite method back in as well
#     this_day_for_vi.append(raw_bands.select('NBR'))
#     this_day_for_select.append(999)
...
# # Use reduceRegion to get the pixel values within the ROI
# data = this_max_snow_day.reduceRegion(
#     reducer=ee.Reducer.toList(),
#     geometry=final_buffer2,
#     scale=scale,
#     maxPixels=1e13
...
# # Convert the NumPy array to a Pandas DataFrame column
# df = pd.DataFrame(data_array, columns= ['test'])
...
# len(this_day_for_select)


# # In[ ]:


# #now loop through and determine for each pixel which day has the maximum dnbr selected


# # In[ ]:


# # for index, image in enumerate(this_day_for_vi):
    
# #     print(index, this_day_for_select[index])

# dnbr_arrays = []

# for index, image in enumerate(this_day_for_vi):
    
#     this_loop_day = str(this_day_for_select[index])
...
# # to_check = pd.concat(dnbr_arrays, axis = 1).dropna().rename(columns=lambda x: int(x))
# to_check = pd.concat(dnbr_arrays, axis = 1).rename(columns=lambda x: int(x))
...
# pd_max = to_check.idxmax(axis = 1)
# # pd_max = df.apply(lambda row: row.argmax(), axis=1)

# to_check['Max_Column'] = pd_max

# # Get a boolean mask where 'Max_Column' is 999
# max_col_999 = to_check['Max_Column'] == 999

# # Count non-NaN values per row, ignoring 'Max_Column'
# non_nan_counts = to_check.drop(columns='Max_Column').notna().sum(axis=1)

# # Identify rows where the only non-NaN value is in column 999
# condition = (non_nan_counts == 1) & to_check[999].notna() & max_col_999

# # Update 'Max_Column' based on the condition
# to_check.loc[condition, 'Max_Column'] = 998
...
# to_check['Max_Column'].unique()


# # In[ ]:


# # Function to add lat/long as bands
...
# # Add lat/long bands to the image
# image_with_latlon = add_lat_long(image)
...
# # Use reduceRegion to get the pixel values within the ROI
# data_lat = image_with_latlon.select('lat').reduceRegion(
#     reducer=ee.Reducer.toList(),
#     geometry=final_buffer2,
#     scale=scale,
#     maxPixels=1e13,
...
# # Extract the dictionary from the NumPy array
# data_dict_lat = np.array([data_lat])[0]

# # Extract the 'NBR' values from the dictionary
# band_data_lat= data_dict_lat['lat']

# # Convert the list of values to a flat NumPy array
# data_array_lat = np.array(band_data_lat)

# # Convert the NumPy array to a Pandas DataFrame column
# df_lat = pd.DataFrame(data_array_lat, columns= ['lat'])
...
#     # Add lat/long bands to the image
#     image_with_latlon = image.addBands(ee.Image.pixelLonLat().select(['latitude', 'longitude']))

#     # Use reduceRegion to get the pixel values within the ROI
#     data = image_with_latlon.reduceRegion(
#         reducer=ee.Reducer.toList(),
#         geometry=final_buffer2,
#         scale=scale,
#         maxPixels=1e13,
...
#     # Extract the 'NBR' and lat/long values from the dictionary
#     band_data = data_dict.get('NBR', [])
#     lat_data = data_dict.get('latitude', [])
#     lon_data = data_dict.get('longitude', [])

#     # Ensure all arrays have the same length
#     min_length = min(len(band_data), len(lat_data), len(lon_data))
#     band_data = band_data[:min_length]
#     lat_data = lat_data[:min_length]
#     lon_data = lon_data[:min_length]

#     # Convert the lists of values to flat NumPy arrays
#     data_array = np.array(band_data)
...
# to_check = pd.concat(dnbr_arrays, axis=1).rename(columns=lambda x: int(x) if x not in ['lat', 'lon'] else x)

# # Ensure that latitude and longitude are not duplicated during concatenation
# lat_lon_cols = to_check[['lat', 'lon']]
# to_check = to_check.drop(columns=['lat', 'lon']).join(lat_lon_cols.iloc[:, :2])

# # Determine the maximum value column
# pd_max = to_check.idxmax(axis=1)
# to_check['Max_Column'] = pd_max
...
# # Print the updated DataFrame with lat and lon
# print(to_check.head())

# # Save to CSV if needed
# # to_check.to_csv('pixel_values_with_max_column_lat_lon.csv', index=False)
...
# # Define the function to process each year and get snow cover images
# def process_year(year):
#     # Define the process for a single year
#     start_date = ee.Date.fromYMD(year, 1, 1)
#     end_date = ee.Date.fromYMD(year, 12, 31)
#     # Assuming process_year returns a collection with a 'calDoy' band
#     return ee.ImageCollection("MODIS/006/MOD10A1") \
#         .filterDate(start_date, end_date) \
#         .map(lambda img: img.addBands(img.date().difference(start_date, 'day').rename('calDoy'))) \
#         .filterBounds(final_buffer) \
#         .median().set('year', year)

# # Define the function to get the VI for a given day
# def get_vi_for_day(day):
#     day = ee.Number(day)
#     start_value = day.add(1)
#     max_value = 244
#     increment = 30

#     list1 = ee.List.sequence(start_value, max_value, increment)
#     # list2 = list1.map(lambda x: ee.Number(x).add(increment))
#     all_days_for_composite = ee.Dictionary.fromLists(list1, list2)

#     vi_images = all_days_for_composite.map(lambda k, v: process_day_interval(ee.Number(k), ee.Number(v)))
#     vi_images_list = ee.List(vi_images.values())

#     # Flatten the list of image collections
#     return ee.ImageCollection(vi_images_list.flatten())

# # Define the function to process each day interval
# def process_day_interval(d1, d2):
#     this_date_pre_start = ee.Date.fromYMD(start_year_snow, 1, 1).advance(d1, 'day')
#     this_date_pre_end = ee.Date.fromYMD(start_year_snow, 1, 1).advance(d2, 'day')
#     this_date_post_start = ee.Date.fromYMD(end_year_snow, 1, 1).advance(d1, 'day')
#     this_date_post_end = ee.Date.fromYMD(end_year_snow, 1, 1).advance(d2, 'day')

#     pre_input_snow = get_image(this_date_pre_start, this_date_pre_end, final_buffer)
#     post_input_snow = get_image(this_date_post_start, this_date_post_end, final_buffer)

#     if pre_input_snow.size().gt(0).And(post_input_snow.size().gt(0)):
#         pre_input_snow = pre_input_snow.median()
#         post_input_snow = post_input_snow.median()
#         raw_bands_snow = pre_input_snow.subtract(post_input_snow).multiply(1000)

#         return raw_bands_snow.select(['NBR', 'NDVI', 'NDII']).updateMask(this_max_snow_day.eq(d1))
#     else:
#         return ee.Image().set('empty', True)

# # Get all unique burn days from the final region
# freqHist = ee.Dictionary(max_snow_day.reduceRegion(
...
# all_days = freqHist.keys()

# # Map the function over all days
# all_days_vi_image = ee.ImageCollection.fromImages(all_days.map(get_vi_for_day)).max()

# # Example export function
# def export_image(image, region, description):
#     task = ee.batch.Export.image.toCloudStorage(
#         image=image.toFloat(),
...
# export_image(all_days_vi_image, final_buffer2, "all_days_vi_image")
